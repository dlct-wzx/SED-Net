    ...
    for train_b_id, data in enumerate(loader_train):   # ====================================> 1000 
        ...
        for _ in range(num_iter):
            ...
            if if_normals:
                ...
                edges_pred = model(points=input)
            else:
                edges_pred = model(points.transpose(1,2))
            ...
            # loss = embed_loss + p_loss + edge_loss +  0.25 * 
        ...
        cur_inter += 1
        print(
            "\rEpoch: {} iter: {}, iou: {}, edge_cls: {}".format(
                e, train_b_id, iou, edge_cls_losses
            ),
            end="",
        )    
        if cur_inter == eval_inter or todebug:
            ...
            for val_b_id, data in enumerate(loader_test):
                ...
                with torch.no_grad():
                    ...
                    loss =  edge_loss = edge_cls_loss(edges_pred, edges, edges_W)
                # 计算测试集 prim类别的iou
                iou = 0
                test_iou.append(iou)
            ...
            my_crition = np.mean(train_edgeBce)
